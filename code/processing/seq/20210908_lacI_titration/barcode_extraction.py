...
def analyze_file(file):
    ...
    seqs = skbio.io.read(
        file, format="fastq", verify="false", variant="illumina1.8"
    )
    ...
    for seq in seqs:
    #for seq in itertools.islice(seqs, 2000):
        if counter % 100000 == 0:
    ...
    file_name = f.split("/")[-1].replace('.fastq.gz', '.csv')
    df_seq.to_csv(f"{outdir}{file_name}", index=False)
