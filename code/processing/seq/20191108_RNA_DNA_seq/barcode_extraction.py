...
datadir = f"{homedir}/data/processed_sequencing/20191108_RNA_DNA_seq/"
# Read CSV file explaining what each file is
metadata = pd.read_csv(
    f"{homedir}/data/demux_sequencing/20191108_RNA_DNA_seq/MANIFEST"
)
metadata = metadata[metadata.direction == 'forward'].reset_index()
metadata = metadata.rename(columns={'sample-id': 'id'})

# output directory
outdir = f"{homedir}/data/barcodes/20191108_RNA_DNA_seq/"
...
for i, f in enumerate(metadata.filename):
    ...
    for i, row in df_seq.iterrows():
        ...
        # Rules for when there is a match
            ...
            gfp_idx.append(match[0][0] - 4)
            gfp_bc.append(row.sequence[match[0][0] - 4 : match[0][0]])
    ...
    bc_bool = [x in df_gfp.gfp_barcode.values for x in df_seq.gfp_bc.values]
    df_seq = df_seq[bc_bool]

    # Keep only desired columns
    df_seq = df_seq[['sequence', 'index', 'op_bc', 'gfp_bc']]
    print('Saving into memory')
    df_seq.to_csv(f"{outdir}{f.replace('.fastq.gz', '.csv')}",
                  index=False)